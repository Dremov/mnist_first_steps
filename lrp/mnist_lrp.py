...
def save_images(outpath, predictions, relevances, images, ys, iter):
    ...
    for h, heat in enumerate(relevances):
        ...
        if (labelCount[label] < 10) and (label != pred):
            ...
            io.imsave(outpath + '/label-' + str(label) + '-idx-' + image_idx + '-predicted-' + str(pred) + '-norm.png', rgb)
            io.imsave(outpath + '/label-' + str(label) + '-idx-' + image_idx + '-predicted-' + str(pred) + '-heat.png', maps)
            io.imsave(outpath + '/label-' + str(label) + '-idx-' + image_idx + '-predicted-' + str(pred) + '-heatn.png', mapsz)
            labelCount[label] += 1
